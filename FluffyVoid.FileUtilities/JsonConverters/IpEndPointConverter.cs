    ...
    public class IpEndPointConverter : JsonConverter
    {
            ...
            if (string.IsNullOrEmpty(jsonString))
            {
                throw new
                    NullReferenceException(
                        "JToken has read in a null or empty string, unable to use data to parse into an IP EndPoint.");
            }

            string[] components = jsonString.Split(':');
            if (components.Length < 2 ||
                !int.TryParse(components[1], out int port))
            {
                return null;
            }
                                       ...
                                       JsonSerializer serializer)
        {
            if (value is not IPEndPoint ipEndPoint)
            {
                throw new
                    ArgumentException(
                        "Data is not an IPEndPoint. Unable to write to JSON.");
            }
